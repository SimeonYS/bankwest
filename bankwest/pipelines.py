...
class BbankwestPipeline:

    # Database setup
    conn = sqlite3.connect('bankwest.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bankwest`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bankwest WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bankwest (date, title, link, content)"
